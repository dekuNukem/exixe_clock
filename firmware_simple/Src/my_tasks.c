...
uint8_t is_in_setup_mode, use_24hour;
int8_t utc_offset;
...
void setup_task(void)
{
  ...
  if(HAL_GPIO_ReadPin(UP_BUTTON_GPIO_Port, UP_BUTTON_Pin) == GPIO_PIN_RESET || HAL_GPIO_ReadPin(DOWN_BUTTON_GPIO_Port, DOWN_BUTTON_Pin) == GPIO_PIN_RESET)
...
void test_task_start(void const * argument)
{
  for(;;)
  {
    if(is_in_setup_mode == SETUP_12H24H)
    {
